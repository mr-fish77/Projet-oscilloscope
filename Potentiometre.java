...
/**
 * Objet permettant l'affichage d'un potentiometre. L'objet herite de JLabel
 * donc on peut y appliquer les memes methodes (hormis setText et getText). Le
 * constructeur par defaut cree un potentiometre normal donc la valeur est
 * nulle.
 */
...
public class Potentiometre extends JLabel implements MouseMotionListener, MouseListener {
	...
	private int taille;
	/**
	 * switch pour affichage de l'octogone normal ou rotationne. Vaut true si
	 * l'octogone est normal, false si l'octogone est penche.
	 */
	...
	/** Definit l'objet qui reagit aux actions du Potentiometre.
	 * @param listener Objet qui implemente PotentiometreListener.
	 */
	public void setPotentiometreListener(PotentiometreListener listener) {
	...
	public void mouseDragged(MouseEvent m) {
		...
		double a = Math.atan2(xMem * y - yMem * x, xMem * x + yMem * y); // on en deduit l'angle par rapport au
																			// precedent cran (formule d'internet)

		if (Math.abs(a) > 1) { // si l'angle est supperieur à 22,5 degre on change de cran (c'est la moitie
								// d'un angle d'octogone)
			if (listener != null) {
